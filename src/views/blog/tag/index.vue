<template>
  <div class="tag-container">
    ...
    <el-table v-loading="loading" border :data="tagList" @selection-change="handleSelectionChange">
      ...
      <!-- 分类名 -->
      <el-table-column align="center" label="标签名" prop="name" width="300" />
      <!-- 颜色 -->
      <el-table-column align="center" label="颜色" prop="color" width="300">
    ...
    <el-dialog append-to-body :title="dialogTitle" :visible.sync="showDialog" width="500px">
      <el-form ref="categoryFormRef" label-width="100px" :model="tagForm" @submit.native.prevent>
        <el-form-item label="分类名称" prop="categoryName">
          <el-input v-model="tagForm.name" placeholder="请输入分类名称" :rules="rules" style="width: 250px" />
        </el-form-item>
...
<script setup>
  ...
  import { getTagList, delTag, insertTag, updateTag } from '@/api/blog'
  ...
  const { loading, tagList, tagForm, dialogTitle, showDialog } = toRefs(data)
  ...
  const openModel = (category) => {
    ...
    if (category) {
      tagForm.value = Object.assign({}, category)
    ...
    categoryFormRef.value?.validate((valid) => {
      if (valid) {
        if (tagForm.value.id) {
          ...
          insertTag(tagForm.value).then(({ data }) => {
